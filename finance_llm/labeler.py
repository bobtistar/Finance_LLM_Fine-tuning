...
SYSTEM_PROMPT = build_claude_classifier_system_prompt()
...
async def classify_sentence(
    ...
    async with semaphore:
        try:
            ...
            parsed = parse_classification_json(raw, allow_null_primary=True)

            if parsed is None or parsed.get("primary") is None:
                return None

            secondary = [
                category
                for category in parsed["secondary"]
                if category in CATEGORY_SET
            ][:2]
            return {"primary": parsed["primary"], "secondary": secondary}
